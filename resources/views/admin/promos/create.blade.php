...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <h2 class="font-serif text-2xl font-bold" style="color: var(--text-primary)">Tambah Promo Baru</h2>
    </div>
    ...
    <form action="{{ route('admin.promos.store') }}" method="POST" class="card rounded-2xl p-6 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                ...
                <input type="number" name="min_order" value="{{ old('min_order', 0) }}" min="0"
                    class="w-full px-4 py-3 rounded-xl border-0"
                    style="background: var(--bg-tertiary); color: var(--text-primary)"
                    placeholder="100000">
            </div>
                ...
                @foreach(['destinasi', 'hotel', 'tiket', 'paket'] as $type)
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="applicable_to[]" value="{{ $type }}" 
                            {{ in_array($type, old('applicable_to', [])) ? 'checked' : '' }}
